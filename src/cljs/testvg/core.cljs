...
(defn about-page []
  [:div [:h2 "About the experience"]
   ...
   [:p "It was interesting to use Reagent as I've mainly used " [:a {:href "https://github.com/tonsky/rum"} "Rum"] " in the past. Rum is less declarative than Reagent but it does mean you can compose your state management abstractions to best suit your own taste, whereas Reagent uses its own atoms and component conventions; for example, Reagent evaluating the function returned by a component immediately means that I couldn't return an invokable function for the parent view to apply as needed, which I've done in Rum previously. I'm not suggesting it's a good idea to do this; it's just that Reagent's choices constrain you more than Rum's do, by design of both libraries."]
   ...
   [:p "One swift email to Arnaud later and I was back on track, using a different data source (the public earthquake dataset Vigiglobe allow clients to use for integration tests.)"]
   ...
   [:p "The linechart shown has a basic mouse-over detailing the closest datapoint, and an average magnitude line (ignoring the practical utility of such a line), but nothing else. This is certainly less than what the linechart component in one of the above libraries gives, but I believe this implementation shows a better understanding of chart creation challenges, including translation from data-space to screen-space, transformation of SVG elements using D3 in clojurescript, and knowledge of associated tasks."]
   [:h3 "On the Vigiglobe Custom Chart tests"]
   [:p "The main idea here was to construct a circular line chart for particular line periods, comparing the intensity of earthquakes over the most recent period against a historical one. For example, the default option shows the current hour's earthquake data vs the same hour yesterday, in minute granularity."]
   [:p "This wasn't completely successful. The data available from the Vigiglobe API often didn't suit the requests (visible in the selection box of available comparisons). In the case of the longer-term datasets, the historical data doesn't seem to be available. If I were to develop this visualisation further, I'd add a mouse-over showing details of individual data points, and more grid lines could be presented to show the difference in periods between the datasets."]
   [:p "More generally, the idea behind the chart is that you could compare social media activity over specific periods, to see at a glance when your normal busy times are (for example, Tuesday afternoons), especially if an average of the previous weeks data was used as a comparison point. Therefore (if developed further) a Vigiglobe customer could make some prediction around the amount of effort required to manage social media particular time periods. The visualisation compares relative intensity only - a strict numerical comparison could have foreshortened the variation in the current or historical line, obscuring the message of the chart."]
