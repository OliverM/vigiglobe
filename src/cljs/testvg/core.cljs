...
(defn about-page []
  [:div [:h2 "About the experience"]
   ...
   [:p "The linechart shown has a basic mouse-over detailing the closest datapoint, and a half-way mark line, but nothing else. This is certainly less than what the linechart component in one of the above libraries gives, but this implementation should show an understanding of understanding chart creation challenges, including translation from data-space to screen-space, transformation of SVG elements using D3 in clojurescript, and knowledge of associated tasks."]
